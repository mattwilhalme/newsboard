  ...
  return withBrowser(async page => {
    ...
    try {
      ...
      if (sourceId === "guardian1") {
        await page.frameLocator('iframe[title="SP Consent Message"]').getByRole("button", { name: "Closer", exact: true })
          .click({ timeout: 2500 }).catch(() => {});
      }
      if (sourceId === "usat1") {
        await page.getByRole("button", { name: "Close Special Offer", exact: true })
          .click({ timeout: 2500 }).catch(() => {});
      }
      hero = await page.evaluate(extractMobileHero, config);
      if (hero.ok) {
        await page.waitForTimeout(500);
        const confirmed = await page.evaluate(extractMobileHero, config);
        if (!confirmed.ok || confirmed.url !== hero.url || confirmed.title !== hero.title) {
          throw new Error("Mobile editorial lead changed while rendering; retry next crawl");
        }
        hero = confirmed;
      }
    } catch (error) { hero = { ok: false, error: error.message }; }
    const item = hero.ok ? {
      title: hero.title, url: normalizeUrl(hero.url), imgUrl: hero.imgUrl,
      slotKey: sha1(sourceId === "usat1" ? "usat|hero" : `${sourceId}|top`).slice(0, 12),
      ...(config.breaking ? { breakingLabel: hero.breakingHeadline ? "Breaking News" : null,
        breakingHeadline: hero.breakingHeadline, breakingUrl: hero.breakingUrl ? normalizeUrl(hero.breakingUrl) : null } : {}),
    } : null;
...
async function scrapeGuardianHero() {
  return scrapeMobileHero("guardian1");
}
...
async function scrapeWPHero() {
  return scrapeMobileHero("yahoo1");
}
