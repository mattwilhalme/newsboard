...
const STORY_GENERIC_WORDS = new Set([
  "killed",
  "shot",
  "says",
  "after",
...
function extractKeywords(titleNorm, maxTokens = 12) {
  ...
  for (const tok of tokens) {
    ...
    if (STORY_GENERIC_WORDS.has(tok)) continue;
...
function extractEntities(title, maxEntities = 10) {
  ...
  for (const chunk of quoted) {
    const phrase = cleanText(chunk.replace(/^["'“”]|["'“”]$/g, ""));
    const norm = phrase.toLowerCase();
    if (!phrase || seen.has(norm)) continue;
    seen.add(norm);
    out.push(phrase);
  ...
  const capMatches = raw.match(/\b(?:[A-Z][a-z0-9]+(?:[-'][A-Z]?[a-z0-9]+)?|[A-Z]{2,})(?:\s+(?:[A-Z][a-z0-9]+(?:[-'][A-Z]?[a-z0-9]+)?|[A-Z]{2,})){1,5}\b/g) || [];
  for (const phraseRaw of capMatches) {
...
function storyFromHeadlineRow(row) {
  ...
  const entities = Array.isArray(raw.entities) && raw.entities.length
    ? raw.entities.map((x) => cleanText(x)).filter(Boolean)
    : extractEntities(row?.title || "");
...
function storyFromScreenshotRow(row) {
  ...
  return {
    ...
    keywords: Array.isArray(raw.keywords) && raw.keywords.length
      ? raw.keywords.map((x) => String(x || "").trim()).filter(Boolean)
      : extractKeywords(title_norm),
    entities: Array.isArray(raw.entities) && raw.entities.length
      ? raw.entities.map((x) => cleanText(x)).filter(Boolean)
      : extractEntities(row?.title || ""),
    fingerprint: String(raw.fingerprint || "").trim() || makeFingerprint(canonical_url, title_norm),
  };
...
function storySimilarity(a, b) {
  const kwA = (a?.keywords || []).filter((k) => !STORY_GENERIC_WORDS.has(String(k || "").toLowerCase()));
  const kwB = (b?.keywords || []).filter((k) => !STORY_GENERIC_WORDS.has(String(k || "").toLowerCase()));
  const entA = uniqStrings(a?.entities || []);
  const entB = uniqStrings(b?.entities || []);
  const kw = jaccardOverlap(kwA, kwB);
  const ent = jaccardOverlap(entA.map((x) => x.toLowerCase()), entB.map((x) => x.toLowerCase()));
  const canonicalMatch = Boolean(a?.canonical_url) && String(a.canonical_url) === String(b?.canonical_url || "");
  const fpMatch = Boolean(a?.fingerprint) && String(a.fingerprint) === String(b?.fingerprint || "");

  let score = 0;
  if (canonicalMatch) score += 1.0;
  if (fpMatch) score += 0.8;
  score += 0.4 * kw.score;
  score += 0.6 * ent.score;

  const dtMs = Math.abs(parseIsoMs(a?.ts || a?.observed_at) - parseIsoMs(b?.ts || b?.observed_at));
  let timeMult = 0.5;
  if (dtMs <= 2 * 60 * 60 * 1000) timeMult = 1.0;
  else if (dtMs <= 6 * 60 * 60 * 1000) timeMult = 0.8;
  score *= timeMult;
  ...
  return {
    ...
    fpMatch,
    kwScore: kw.score,
    kwOverlap: kw.overlap,
    entScore: ent.score,
    entOverlap: ent.overlap,
  };
}
...
function pickClusterVerb(stories = []) {
  ...
  for (const story of stories) {
    const tokens = String(story?.title_norm || "").split(/\s+/g).filter(Boolean);
    for (const tok of tokens) {
      if (!verbs.has(tok)) continue;
...
function buildClusterTitle(stories = []) {
  const entityCounts = new Map();
  for (const story of stories) {
    for (const ent of uniqStrings(story?.entities || [])) {
      const k = ent.toLowerCase();
      entityCounts.set(k, { phrase: ent, count: (entityCounts.get(k)?.count || 0) + 1 });
    }
  }
  const entities = [...entityCounts.values()].sort((a, b) => {
    if (b.count !== a.count) return b.count - a.count;
    return String(b.phrase || "").length - String(a.phrase || "").length;
  });
  const topEntities = entities.slice(0, 2).map((x) => x.phrase);
  const verb = pickClusterVerb(stories);
  if (topEntities.length) {
    const base = topEntities.join(" · ");
  ...
  for (const story of stories) {
    const tokens = String(story?.title_norm || "").split(/\s+/g).filter(Boolean);
    for (const tok of tokens) {
      if (tok.length < 4 || STORY_STOPWORDS.has(tok) || STORY_GENERIC_WORDS.has(tok)) continue;
      tf.set(tok, (tf.get(tok) || 0) + 1);
    }
  }
  const ranked = [...tf.entries()].sort((a, b) => a[1] - b[1] || b[0].length - a[0].length);
  if (!ranked.length) return "Storyline";
  const phrase = ranked.slice(0, 3).map((x) => x[0]).join(" ");
  return phrase.charAt(0).toUpperCase() + phrase.slice(1);
}

function clusterStories(stories = [], opts = {}) {
  const maxPairHours = Number.isFinite(Number(opts.maxPairHours)) ? Number(opts.maxPairHours) : 24;
  const maxClusterSpanHours = Number.isFinite(Number(opts.maxClusterSpanHours))
    ? Number(opts.maxClusterSpanHours)
    : (opts.storylineMode ? 72 : 24);
  const edgeScore = Number.isFinite(Number(opts.edgeScore)) ? Number(opts.edgeScore) : 0.85;
  const edgeScoreSoft = Number.isFinite(Number(opts.edgeScoreSoft)) ? Number(opts.edgeScoreSoft) : 0.70;
  const sorted = [...stories]
    .map((story, idx) => ({ ...story, __idx: idx, ts: story?.ts || story?.observed_at || null }))
    .filter((story) => parseIsoMs(story.ts) > 0)
    .sort((a, b) => parseIsoMs(a.ts) - parseIsoMs(b.ts));
  if (!sorted.length) return [];

  const parent = new Array(sorted.length).fill(0).map((_, i) => i);
  const find = (x) => {
    let p = x;
  ...
  const maxPairMs = maxPairHours * 60 * 60 * 1000;
  ...
  for (let i = 0; i < sorted.length; i += 1) {
    ...
    byRoot.get(root).push(sorted[i]);
  }

  const clusters = [];
  const maxSpanMs = maxClusterSpanHours * 60 * 60 * 1000;
  for (const rows of byRoot.values()) {
    const ordered = [...rows].sort((a, b) => parseIsoMs(a.ts) - parseIsoMs(b.ts));
    if (!ordered.length) continue;
    if (!maxSpanMs || maxSpanMs <= 0) {
      clusters.push(ordered);
      continue;
    }
    let chunk = [];
    let chunkStart = parseIsoMs(ordered[0].ts);
  ...
  return clusters
    .map((storiesInCluster, idx) => {
      const ordered = [...storiesInCluster].sort((a, b) => parseIsoMs(a.ts) - parseIsoMs(b.ts));
      const entitySet = new Set();
      const keywordSet = new Set();
      for (const row of ordered) {
        for (const ent of uniqStrings(row.entities || [])) entitySet.add(ent);
        for (const kw of row.keywords || []) keywordSet.add(String(kw || "").toLowerCase());
      }
      return {
        id: `cluster_${idx + 1}`,
        title: buildClusterTitle(ordered),
        stories: ordered,
        start_at: ordered[0]?.ts || null,
        end_at: ordered[ordered.length - 1]?.ts || null,
        entities: [...entitySet].slice(0, 10),
        keywords: [...keywordSet].slice(0, 12),
      };
    })
    .sort((a, b) => parseIsoMs(a.start_at) - parseIsoMs(b.start_at));
}
...
app.get("/api/storyline/build", async (req, res) => {
  try {
    if (!hasSupabaseAdmin()) {
      return res.status(503).json({ ok: false, error: "Supabase admin is not configured for storyline build." });
    }

    const sb = getSupabaseAdmin();
    const sourceId = String(req.query?.source_id || req.query?.source || "").trim().toLowerCase();
    const urlRaw = String(req.query?.url || "").trim();
    const canonicalFromUrl = normalizeUrl(urlRaw);
    const titleRaw = String(req.query?.title || "").trim();
    const t0Raw = String(req.query?.t0 || "").trim();
    const t0ProvidedMs = parseIsoMs(t0Raw);
    const lookbackMinutes = Number.isFinite(Number(req.query?.lookback_minutes))
      ? Math.max(15, Math.min(12 * 60, Math.floor(Number(req.query.lookback_minutes))))
      : 120;
    const extendMinutes = Number.isFinite(Number(req.query?.extend_minutes))
      ? Math.max(15, Math.min(12 * 60, Math.floor(Number(req.query.extend_minutes))))
      : 120;
    const bucketMinutes = Number.isFinite(Number(req.query?.bucket_minutes))
    ...
    const silenceGapMs = 90 * 60 * 1000;

    if (!sourceId) return res.status(400).json({ ok: false, error: "source_id is required." });
    if (!canonicalFromUrl) return res.status(400).json({ ok: false, error: "url is required." });

    async function fetchHeadlineRange(startIso, endIso, limit = maxRows) {
      ...
      const rows = (Array.isArray(data) ? data : []).filter((row) => {
        ...
        if (slot === "hero:1") return true;
        if (!includeTop10) return slot === "top10:1";
        return /^top10:\d+$/i.test(slot);
      });
      return rows.map((row) => {
        ...
        return {
          ...s,
          ts: s.observed_at,
          rank: parseRankFromSlot(s.slot_key, s.slot_rank),
          slot: s.slot_key,
        };
      });
    }
    ...
    async function fetchScreenshotsRange(startIso, endIso, limit = 1000) {
      ...
      if (error) {
        ...
        shots = Array.isArray(fallbackData) ? fallbackData : [];
      } else {
      ...
      if (!SUPABASE_SCREENSHOT_PUBLIC) {
        const uniquePaths = [...new Set(shots.map((row) => row?.object_path).filter(Boolean))];
        const signedByPath = new Map();
        for (let i = 0; i < uniquePaths.length; i += 100) {
          const batch = uniquePaths.slice(i, i + 100);
      ...
      return shots.map((row) => storyFromScreenshotRow(row));
    }

    // A) Anchor
    let anchorStory = null;
    const t0BaseMs = t0ProvidedMs || Date.now();
    const latestAnchorCutoff = new Date(t0BaseMs - (24 * 60 * 60 * 1000)).toISOString();
    const latestAnchorLimit = new Date(t0BaseMs + (1 * 60 * 60 * 1000)).toISOString();
    const anchorRows = await fetchHeadlineRange(latestAnchorCutoff, latestAnchorLimit, 2500);
    const sourceRows = anchorRows.filter((row) => String(row.source_id || "") === sourceId);

    if (t0ProvidedMs) {
      const atOrBefore = sourceRows
        .filter((row) => parseIsoMs(row.ts) <= t0ProvidedMs)
        .sort((a, b) => parseIsoMs(b.ts) - parseIsoMs(a.ts));
      anchorStory = atOrBefore.find((row) => String(row.canonical_url || "") === canonicalFromUrl) || atOrBefore[0] || null;
    } else {
      const sortedRecent = sourceRows.sort((a, b) => parseIsoMs(b.ts) - parseIsoMs(a.ts));
      anchorStory = sortedRecent.find((row) => String(row.canonical_url || "") === canonicalFromUrl) || sortedRecent[0] || null;
    }
    ...
    if (!anchorStory) {
      ...
      anchorStory = {
        source_id: sourceId,
        ts: new Date(t0BaseMs).toISOString(),
        observed_at: new Date(t0BaseMs).toISOString(),
        title: titleRaw || null,
        url: urlRaw || null,
        canonical_url: canonicalFromUrl || null,
        title_norm: titleNorm || null,
        keywords: extractKeywords(titleNorm),
        entities: extractEntities(titleRaw || ""),
        fingerprint: makeFingerprint(canonicalFromUrl, titleNorm),
        slot: "hero:1",
        slot_key: "hero:1",
        rank: 1,
      };
    }
    const t0Ms = t0ProvidedMs || parseIsoMs(anchorStory.ts);
    const t0Iso = new Date(t0Ms).toISOString();

    const anchorSignature = {
      canonical_url: anchorStory.canonical_url || canonicalFromUrl || null,
      fingerprint: anchorStory.fingerprint || makeFingerprint(canonicalFromUrl, normalizeTitle(anchorStory.title || titleRaw || "")),
      title_norm: anchorStory.title_norm || normalizeTitle(anchorStory.title || titleRaw || ""),
      keywords: anchorStory.keywords && anchorStory.keywords.length
        ? anchorStory.keywords
        : extractKeywords(anchorStory.title_norm || ""),
      entities: anchorStory.entities && anchorStory.entities.length
        ? anchorStory.entities
        : extractEntities(anchorStory.title || titleRaw || ""),
      ts: anchorStory.ts,
    };

    // B) Lookback + cluster match for primary start.
    const lookbackStartMs = t0Ms - (lookbackMinutes * 60 * 1000);
    const lookbackStories = await fetchHeadlineRange(new Date(lookbackStartMs).toISOString(), t0Iso, maxRows);
    const lookbackClusters = clusterStories(lookbackStories, {
      maxPairHours: 2,
      maxClusterSpanHours: 2,
      edgeScore: 0.85,
      edgeScoreSoft: 0.70,
      storylineMode: true,
    });
    ...
    for (const cluster of lookbackClusters) {
      let bestStoryScore = Number.NEGATIVE_INFINITY;
      for (const story of cluster.stories) {
        const matched = isSignatureMatch(story, anchorSignature);
        if (!matched.match) continue;
        let score = matched.sim.score;
        if (matched.sim.canonicalMatch) score += 0.6;
        if (matched.sim.fpMatch) score += 0.4;
        if (score > bestStoryScore) bestStoryScore = score;
      }
      if (bestStoryScore > bestClusterScore) {
        bestClusterScore = bestStoryScore;
        selectedCluster = cluster;
      }
    }

    if (!selectedCluster) {
      const directMatches = lookbackStories.filter((story) => isSignatureMatch(story, anchorSignature).match);
      selectedCluster = {
        id: "direct",
        title: buildClusterTitle(directMatches.length ? directMatches : [anchorStory]),
        stories: directMatches.length ? directMatches : [anchorStory],
        start_at: (directMatches[0]?.ts || anchorStory.ts),
        end_at: (directMatches[directMatches.length - 1]?.ts || anchorStory.ts),
        entities: uniqStrings(anchorSignature.entities || []).slice(0, 10),
        keywords: uniqStrings(anchorSignature.keywords || []).slice(0, 12),
      };
    }

    let primaryEvent = [...selectedCluster.stories].sort((a, b) => parseIsoMs(a.ts) - parseIsoMs(b.ts))[0] || anchorStory;
    let startAtMs = parseIsoMs(primaryEvent.ts) || t0Ms;

    // C) Backward extension.
    const minBackMs = Math.max(0, t0Ms - maxBackMs);
    for (let iter = 0; iter < 24; iter += 1) {
      const winStart = Math.max(minBackMs, startAtMs - (extendMinutes * 60 * 1000));
      if (winStart >= startAtMs) break;
      const rows = await fetchHeadlineRange(new Date(winStart).toISOString(), new Date(startAtMs).toISOString(), maxRows);
      const matches = rows.filter((story) => isSignatureMatch(story, anchorSignature).match);
      if (!matches.length) break;
      const earliest = parseIsoMs(matches[0].ts);
      if (earliest >= startAtMs) break;
      startAtMs = earliest;
      primaryEvent = matches[0];
      if (startAtMs <= minBackMs) break;
    }

    // D) Forward extension until silence gap.
    let cursorMs = t0Ms;
    const maxForwardEdge = t0Ms + maxForwardMs;
    let lastMatchMs = Math.max(startAtMs, t0Ms);
    for (let iter = 0; iter < 36 && cursorMs < maxForwardEdge; iter += 1) {
      const nextMs = Math.min(maxForwardEdge, cursorMs + (extendMinutes * 60 * 1000));
      const rows = await fetchHeadlineRange(new Date(cursorMs).toISOString(), new Date(nextMs).toISOString(), maxRows);
      const matches = rows.filter((story) => isSignatureMatch(story, anchorSignature).match);
      if (matches.length) {
        lastMatchMs = parseIsoMs(matches[matches.length - 1].ts);
      } else if ((nextMs - lastMatchMs) > silenceGapMs) {
        break;
      }
      cursorMs = nextMs;
    }
    const endAtMs = Math.min(maxForwardEdge, lastMatchMs + (5 * 60 * 1000));
    ...
    let bestStoryCount = 0;
    for (const cluster of storyClusters) {
      const containsAnchor = cluster.stories.some((story) => String(story.source_id || "") === sourceId && parseIsoMs(story.ts) <= t0Ms + 1000);
      const count = cluster.stories.length;
      const score = count + (containsAnchor ? 10 : 0);
      if (score > bestStoryCount) {
        bestStoryCount = score;
        bestCluster = cluster;
      }
    }
    const clusterSignature = bestCluster || {
      title: buildClusterTitle(matchedRows.length ? matchedRows : [anchorStory]),
      entities: uniqStrings(anchorSignature.entities || []).slice(0, 10),
      keywords: uniqStrings(anchorSignature.keywords || []).slice(0, 12),
      stories: matchedRows,
    };
    ...
    for (const story of matchedRows) {
      const key = bucketKey(story.ts);
      if (!key) continue;
      if (!bucketMap.has(key)) bucketMap.set(key, { ts: key, coverageBySource: new Map(), screenshots: [] });
      const bucket = bucketMap.get(key);
      const source = String(story.source_id || "");
      const prev = bucket.coverageBySource.get(source) || null;
      const candidate = {
        source_id: source,
        source_name: sourceNameById(source),
        slot: story.slot || story.slot_key || null,
        rank: parseRankFromSlot(story.slot || story.slot_key, story.rank),
      ...
      if (!prev) {
        bucket.coverageBySource.set(source, candidate);
        continue;
      }
      const prevHero = String(prev.slot || "") === "hero:1";
      const nextHero = String(candidate.slot || "") === "hero:1";
      if (nextHero && !prevHero) {
        bucket.coverageBySource.set(source, candidate);
        continue;
      }
      if (prevHero && !nextHero) continue;
      const prevRank = Number(prev.rank);
      const nextRank = Number(candidate.rank);
      if (!Number.isFinite(prevRank) || (Number.isFinite(nextRank) && nextRank < prevRank)) {
        bucket.coverageBySource.set(source, candidate);
      }
    }
    ...
    const buckets = [...bucketMap.values()]
      .map((bucket) => ({
        ts: bucket.ts,
        coverage: [...bucket.coverageBySource.values()].sort((a, b) => String(a.source_id || "").localeCompare(String(b.source_id || ""))),
        screenshots: bucket.screenshots.sort((a, b) => parseIsoMs(a.ts) - parseIsoMs(b.ts)),
        events: [],
      }))
      .sort((a, b) => parseIsoMs(a.ts) - parseIsoMs(b.ts));
    ...
    for (const bucket of buckets) {
      const events = [];
      const prevBySource = new Map((prevBucket?.coverage || []).map((row) => [row.source_id, row]));
      const curBySource = new Map((bucket?.coverage || []).map((row) => [row.source_id, row]));
      const entered = [...curBySource.keys()].filter((id) => !prevBySource.has(id));
      const exited = [...prevBySource.keys()].filter((id) => !curBySource.has(id));
      if (entered.length) events.push({ code: "ENTER", sources: entered });
      if (exited.length) events.push({ code: "EXIT", sources: exited });

      const moved = [];
      const edits = [];
      for (const [source, cur] of curBySource.entries()) {
        const prev = prevBySource.get(source);
        if (!prev) continue;
        const pr = Number(prev.rank);
        const cr = Number(cur.rank);
        if (Number.isFinite(pr) && Number.isFinite(cr) && pr !== cr) {
          moved.push({ source_id: source, from_rank: pr, to_rank: cr, title: cur.title || null });
        }
        const sameStory = (cur.canonical_url && prev.canonical_url && cur.canonical_url === prev.canonical_url)
          || (cur.fingerprint && prev.fingerprint && cur.fingerprint === prev.fingerprint);
        if (sameStory && String(cur.title || "") !== String(prev.title || "")) {
          edits.push({ source_id: source, from_title: prev.title || null, to_title: cur.title || null });
        }
      }
      if (moved.length) events.push({ code: "MOVE", items: moved });
      if (edits.length) events.push({ code: "EDIT", items: edits });
      bucket.events = events;
      bucket.coverage_count = bucket.coverage.length;
      prevBucket = bucket;
    }
    ...
    return res.json({
      ...
      anchor: {
        ...
        url: anchorStory.url || urlRaw || null,
        ts: t0Iso,
      },
      primary: {
        title: primaryEvent?.title || null,
        url: primaryEvent?.url || null,
        ts: new Date(startAtMs).toISOString(),
        reason: startAtMs < t0Ms ? "earliest_match" : "anchor",
      },
      t0: t0Iso,
      start_at: startAtIso,
      end_at: endAtIso,
      cluster: {
        title: clusterSignature.title || buildClusterTitle(clusterSignature.stories || []),
        entities: uniqStrings(clusterSignature.entities || []).slice(0, 10),
        keywords: uniqStrings(clusterSignature.keywords || []).slice(0, 12),
        canonical_url: anchorSignature.canonical_url || null,
        fingerprint: anchorSignature.fingerprint || null,
      },
      buckets,
    });
  } catch (err) {
