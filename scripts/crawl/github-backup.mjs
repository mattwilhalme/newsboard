...
const collectors={abc1:scrapers.scrapeABCHero,cbs1:scrapers.scrapeCBSHero,usat1:scrapers.scrapeUSATHero,nbc1:scrapers.scrapeNBCHero,cnn1:scrapers.scrapeCNNHero,guardian1:scrapers.scrapeGuardianHero,ap1:scrapers.scrapeAPHero,latimes1:scrapers.scrapeLATimesHero,npr1:scrapers.scrapeNPRHero,bbc1:scrapers.scrapeBBCHero,fox1:scrapers.scrapeFoxHero,yahoo1:scrapers.scrapeWPHero};
...
try{
 for(const [source_id,collect] of Object.entries(collectors)){
  ...
  try{
   ...
   if(!result.ok||!result.item?.url||!result.item?.title) throw new Error(result.error||'No usable CP');
   const item=result.item;
   output={source_id,observed_at:result.updatedAt||new Date().toISOString(),item,items:[{...item,rank:1,slot_key:'hero:1',fingerprint:createHash('sha1').update(`${item.url}|${item.title}`).digest('hex')}],http_status:result.meta?.http_status||null};
   if(source_id==='abc1'){
    const top=await scrapers.scrapeABCTop10();
    if(!top.ok||top.items.length!==10) throw new Error(top.error||'Incomplete ABC Top 10');
    output.top10=top.items;
    output.items=top.items.map(item=>({...item,slot_key:`top10:${item.rank}`}));
   }
  }catch(e){error=e.message;}
  const {error:saveError}=await db.rpc('newsboard_save_source',{p:{run_id:id,source_id,started_at,completed_at:new Date().toISOString(),method:'browser',success:!error,error,output}});
